...
class ChangelogReadMore {
    ...
    init() {
        ...
        if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ()=>this.setupBlocks());
        else this.setupBlocks();
    }
    setupBlocks() {
    ...
    truncateHtmlPreservingTags(htmlContent, wordLimit) {
        ...
        if (truncated && truncationNode) this.removeElementsAfterTruncation(tempDiv, truncationNode);
    ...
    removeElementsAfterTruncation(container, truncationNode) {
        ...
        while(currentElement && currentElement !== container){
            ...
            nextSiblings.forEach((sibling)=>{
                if (sibling.nodeType === Node.ELEMENT_NODE) sibling.remove();
            });
        ...
        if (parent) {
            ...
            while(node = walker.nextNode()){
                ...
                if (foundTruncationNode) // Remove this text node
                node.remove();
            }
        }
    }
    ...
    replaceTextContent(block, newText) {
        ...
        const textNodes = [];
        let node;
        while(node = walker.nextNode())// Skip text nodes that are part of the read more button
        if (!node.parentElement.classList.contains('read_more_btn')) textNodes.push(node);
        ...
        this.changelogBlocks.forEach((block)=>{
            ...
            if (readMoreBtn && richTextElement && originalHtml) {
                ...
                if (words.length > 100) {
                    ...
                    richTextElement.innerHTML = truncatedHtml;
                    block.setAttribute('data-expanded', 'false');
                } else block.setAttribute('data-expanded', 'true');
            }
            // Reset block styles
            block.style.height = 'auto';
            block.style.overflow = '';
    ...
    expandAllBlocks() {
        this.changelogBlocks.forEach((block)=>{
            const readMoreBtn = block.querySelector('.read_more_btn');
            if (readMoreBtn && block.getAttribute('data-expanded') === 'false') this.expandBlock(block, readMoreBtn);
        });
    }
}
    ...
    const observer = new IntersectionObserver((entries)=>{
        entries.forEach((entry)=>{
            if (entry.isIntersecting) {
                ...
                if (block.getAttribute('data-setup') !== 'true') // Re-setup if needed
...
if (module.exports) module.exports = ChangelogReadMore;

},{}]},["3O61n","6rimH"], "6rimH", "parcelRequirece4a", {})
